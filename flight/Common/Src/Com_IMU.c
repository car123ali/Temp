                              ...
                              float dt)
{
    ...
    // 加速度归一化
    NormQuat = Q_rsqrt(squa(gyroAccel->a.x) +
                       squa(gyroAccel->a.y) +
                       squa(gyroAccel->a.z));

    Acc.x = gyroAccel->a.x * NormQuat;
    Acc.y = gyroAccel->a.y * NormQuat;
    Acc.z = gyroAccel->a.z * NormQuat;
    // 向量差乘得出的值
    ...
    // 角速度融合加速度积分补偿值
    Gyro.x = gyroAccel->g.x * Gyro_Gr + KpDef * AccGravity.x + GyroIntegError.x;
    // 弧度制
    Gyro.y = gyroAccel->g.y * Gyro_Gr + KpDef * AccGravity.y + GyroIntegError.y;
    Gyro.z = gyroAccel->g.z * Gyro_Gr + KpDef * AccGravity.z + GyroIntegError.z;
    ...
    float yaw_G = gyroAccel->g.z * Gyro_G;
    ...
    normAccz = gyroAccel->a.x * vecxZ + gyroAccel->a.y * vecyZ + gyroAccel->a.z * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
}
